...
// Master settings hub — owner or admin only
if (!has_role(['owner', 'admin'])) {
    flash('error', 'Access denied. Settings management requires admin or owner access.');
    redirect('index');
}
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
    ...
    <!-- Card 2: Chart of Accounts & General Ledger -->
    <a href="accounts" class="group bg-white rounded-2xl p-6 border border-slate-200 shadow-sm hover:shadow-xl hover:border-indigo-500/50 transition-all">
        <div class="flex items-center justify-between mb-4">
            ...
            <span class="text-3xs font-extrabold px-2 py-1 bg-indigo-100 text-indigo-800 rounded-md uppercase">General Ledger</span>
        </div>
        <h3 class="font-extrabold text-base text-slate-900 group-hover:text-indigo-600 transition-colors">Chart of Accounts (COA)</h3>
        <p class="text-xs text-slate-500 mt-1">Manage general ledger codes, assets, liabilities, equity, revenues, and double-entry journal entries.</p>
